...
def test_declare_then_put_roundtrip(client):
    ...
    r = client.post("/v1/media/input", json={"url": "media://sess/a.wav"})
    ...
    r2 = client.put(path, content=b"RIFFDATA")
...
def test_declare_requires_url(client):
    r = client.post("/v1/media/input", json={})
    assert r.status_code == 400


def test_declare_rejects_bad_key(client):
    r = client.post("/v1/media/input", json={"url": "media://../etc/passwd"})
    assert r.status_code == 400


def test_put_empty_body_rejected(client):
    client.post("/v1/media/input", json={"url": "media://k.wav"})
    r = client.put("/v1/media/input/k.wav", content=b"")
    assert r.status_code == 400
