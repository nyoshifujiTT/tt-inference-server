...
**Both forms are capped, and inline is capped lower.** Fetched audio (`media://`, `http(s)://`) is bounded by `MEDIA_URL_MAX_BYTES` — **1 GiB**, matching [what the pyannoteAI cloud API accepts](https://docs.pyannote.ai/support/faqs) for a diarization job, so a client that works against the official service is not refused here for a reason the official service would not have refused it. Inline base64 is bounded by `MEDIA_INLINE_MAX_BYTES`, **64 MiB**. Inline is refused on the encoded length before it is decoded; a fetched object is refused on the declared `Content-Length` when the store sends one, and otherwise mid-stream once the bytes read pass the cap. Neither limit can be dodged by switching form: staging an over-large file just moves the refusal from the decoder to the downloader.

*Why inline is held to less, when the official limit is 1 GiB:* the two paths do not cost the server the same, and the inline cap does a different job.

A fetched object is streamed into a single buffer and checked as it arrives, so an over-large one is abandoned mid-stream. An inline body cannot be: the ASGI layer has to receive and parse the whole request before any handler exists to measure it. Measured on a p150 in a 6 GiB container (the allotment `whisper-large-v3` gets):

- a **1024 MiB** inline body that the cap **rejected** still cost **+1289 MiB** RSS — the memory was already spent before the 413 was decided;
- a **900 MiB** inline body, *inside* a 1 GiB cap, reached the pipeline and the container was **OOM-killed** (`uvicorn ... Killed`, exit 137).

So the inline cap is not really a limit on what gets processed — it is a limit on what a single request can make the server hold before it has any say. That is why it cannot be the official 1 GiB even though the fetched cap can: at 1 GiB the container dies either way, whether the request is accepted or refused. At 64 MiB the base64 form is 85 MiB, a ~0.25 GiB peak against ~4.4 GiB of headroom, so several concurrent requests still fit.

64 MiB covers ~35 minutes of PCM16 inline. Longer audio wants the url path, which is the only form the official API takes anyway. Set `MEDIA_INLINE_MAX_BYTES=0` to opt out and follow `MEDIA_URL_MAX_BYTES` instead, if you would rather tune one number.
...
| inline base64 | `MEDIA_INLINE_MAX_BYTES` | 64 MiB | ~35 min of 16 kHz mono PCM16. Held below the fetched cap because the body is resident **before** the cap can be applied — see below. |
