...
def _is_bge_context() -> bool:
    ...
    return (
        "bge-m3" in haystack
        or "baai/bge-m3" in haystack
        or "bge-reranker-v2-m3" in haystack
    )
...
def _patch_bge_tt_platform_arch_prefix() -> None:
    ...
    def _normalize_arches(vllm_config):
        ...
        for i, arch in enumerate(list(arch_names)):
            ...
            elif arch == "TTXLMRobertaForSequenceClassification":
                arch_names[i] = "XLMRobertaForSequenceClassification"
        return arch_names
...
def _patch_bge_roberta_embedding_shim() -> None:
    ...
    def _initialize_vllm_model(cls, *args, **kwargs):
        ...
        return BgeRerankerV2M3.initialize_vllm_model(*args, **kwargs)
...
def _patch_v1_tt_model_runner_pooling_tasks_for_embedding() -> None:
    ...
    def _patched(self):
        ...
        logger.warning(
            "sitecustomize: forcing TTModelRunner supported pooling tasks to ['embed', 'score', 'classify'] in bge context"
        )
        return ["embed", "score", "classify"]
...
def _patch_v1_tt_model_runner_execute_for_embedding() -> None:
    ...
    # Safety default: this fast-path can trigger device hangs on BH-LB.
    # Keep disabled unless explicitly enabled for experiments.
    if os.getenv("TT_BGE_ENABLE_EMBED_FASTPATH", "0") != "1":
        logger.warning(
            "sitecustomize: skipping v1 TTModelRunner execute_model embedding fast-path (set TT_BGE_ENABLE_EMBED_FASTPATH=1 to enable)"
        )
        return
