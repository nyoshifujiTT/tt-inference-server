...
# Build vllm (canonical path): upstream vLLM 0.24.0 with VLLM_TARGET_DEVICE=empty
# plus the standalone Tenstorrent plugin (tenstorrent/vllm-tt-plugin). Unlike the
# legacy fork path, the "tt" platform, model registry and the TT pooling runner
# are provided by the plugin itself through vLLM's standard entry-points
# (vllm.platform_plugins / vllm.general_plugins) -- no vLLM core patch and no
# sitecustomize monkey-patch. TT_VLLM_COMMIT_SHA_OR_TAG pins the plugin branch.
RUN /bin/bash -c "git clone https://github.com/nyoshifujiTT/vllm-tt-plugin.git ${vllm_dir} \
    && cd ${vllm_dir} \
    && git checkout ${TT_VLLM_COMMIT_SHA_OR_TAG} \
    && source ${PYTHON_ENV_DIR}/bin/activate \
    && export VLLM_TARGET_DEVICE=empty \
    && uv pip install --upgrade pip \
    && uv pip install --index-strategy unsafe-best-match --no-binary vllm vllm==0.24.0 --extra-index-url https://download.pytorch.org/whl/cpu \
...
# Copy EXTRA_MODELS_DIR bundles (per-model vllm_metadata.json). The standalone
# plugin scans this at startup and registers TT<arch> -> module:Class, so models
# like bge-reranker-v2-m3 need no plugin-source edit and no sitecustomize.
COPY --chown=container_app_user:container_app_user \
    "vllm-tt-metal/extra_models" "/home/container_app_user/extra_models"
