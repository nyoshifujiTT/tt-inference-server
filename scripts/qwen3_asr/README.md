...
docker buildx bake -f dockerfile/docker-bake.hcl \
  --set ci-build.tags=local/tt-metal/tt-metalium/ubuntu-22.04-amd64:3041fab \
...
TT_METAL_REPO_URL=https://github.com/nyoshifujiTT/tt-metal.git \
  python3 scripts/build_docker_images.py --build-metal-commit 3041fab --single-threaded
```
...
python3 run.py --model Qwen3-ASR-1.7B-JA --tt-device p150 --workflow server \
  ...
  --override-docker-image ghcr.io/tenstorrent/tt-inference-server/vllm-tt-metal-src-dev-ubuntu-22.04-amd64:0.13.0-3041fab-5e69638
```
