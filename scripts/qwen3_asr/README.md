...
docker buildx bake -f dockerfile/docker-bake.hcl \
  --set ci-build.tags=local/tt-metal/tt-metalium/ubuntu-22.04-amd64:319f413 \
...
TT_METAL_REPO_URL=https://github.com/nyoshifujiTT/tt-metal.git \
  python3 scripts/build_docker_images.py --build-metal-commit 319f413 --single-threaded
```
...
python3 run.py --model Qwen3-ASR-1.7B-JA --tt-device p150 --workflow server \
  ...
  --override-docker-image ghcr.io/tenstorrent/tt-inference-server/vllm-tt-metal-src-dev-ubuntu-22.04-amd64:0.13.0-319f413-5e69638
```
