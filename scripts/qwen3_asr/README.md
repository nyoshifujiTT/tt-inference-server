...
docker buildx bake -f dockerfile/docker-bake.hcl \
  --set ci-build.tags=local/tt-metal/tt-metalium/ubuntu-22.04-amd64:157cbad79865825ea1c80966b29b68468110c783 \
...
+  version: "0.1.0"
+  tt_metal_commit: "157cbad79865825ea1c80966b29b68468110c783"
+  vllm_commit: "4a93161"
+  impl: tt_vllm_plugin
+  min_disk_gb: 15
+  min_ram_gb: 6
...
python3 scripts/build_docker_images.py --build-metal-commit 157cbad79865825ea1c80966b29b68468110c783 --single-threaded
...
MODEL_SPECS_ENV=dev python3 run.py --model Qwen3-ASR-1.7B-JA --tt-device p150 \
  ...
  --host-hf-cache \
  --override-docker-image ghcr.io/tenstorrent/tt-inference-server/vllm-tt-metal-src-dev-ubuntu-22.04-amd64:0.21.0-157cbad79865825ea1c80966b29b68468110c783-4a93161
```
...
| `SNAP` | the HF hub snapshot of `neosophie/Qwen3-ASR-1.7B-JA`, **by revision SHA** (`987bda16...`) | passed as `MODEL_WEIGHTS_DIR` **with `MODEL_SOURCE=local` and `--host-weights-dir`** -- all three are needed, because `setup_host.py` reads `MODEL_WEIGHTS_DIR` only on the local-source branch and `model_source` defaults to `huggingface`. Without them the pinned revision was decorative: the path guard passed and `run.py` still resolved the repo through the HF cache. The SHA is literal, not globbed, so a re-download at a newer revision makes the path missing and the guard exits before launch -- override `SNAP` then, rather than editing the script |
